...
function buildVscodeTasksConfig(): string {
...
function buildManuscriptConfig(): string {
  return [
    ...
    "page_size: letter",
    "",
    "# Custom frontmatter fields to display under chapter/paragraph headings.",
    "# Use show_custom_<field>: <order> where <field> is a frontmatter key",
    "# and <order> is a number controlling display order (lower = first).",
    "# Example: show_custom_date: 10 will display the 'date' field from frontmatter.",
    "# show_custom_date: 10",
    "# show_custom_location: 20",
    "",
  ].join("\n");
}

function buildManuscriptScript(): string {
  return [
    '#!/usr/bin/env python3',
    '"""',
    ...
    '',
    '',
    'def add_custom_frontmatter_fields(doc, metadata, settings):',
    '    customs = []',
...
function buildManuscriptRequirements(): string {
  return [
    "python-docx>=1.1.0",
    "PyYAML>=6.0",
    "",
...
function buildManuscriptRunner(): string {
  return [
    "#!/usr/bin/env node",
