...
- \`ghostwriters/<slug>.md\` for complete prose profiles; \`book.md\`, chapters, paragraphs, drafts, or scripts select one with \`ghostwriter\`
...
- Before writing a scene, review \`context.md\`, \`story-design.md\`, \`notes.md\`, any matching chapter draft notes, the relevant prior chapter content, the latest scoped summaries in \`resumes/\`, the selected ghostwriter, the current point-in-time snapshot in \`state/\` when available, and any matching files in \`drafts/\`.
