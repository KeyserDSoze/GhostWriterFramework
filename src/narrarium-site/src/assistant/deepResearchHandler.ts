...
export async function executeDeepResearchFromCopilot(
  ...
  return runner({ settings: input.settings, book: input.book, branch: input.branch, token: input.token, query: request.query, depth: request.depth, intents: request.intents, language: input.structureLanguage ?? input.settings.ui.language, signal: input.signal, onProgress: input.onText });
}
