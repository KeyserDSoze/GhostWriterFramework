...
function completionEvidenceIsExact(completion: RemovalCompletion): boolean {
  if (!completion.journalId || !completion.localInstanceId || !completion.receiptId || !completion.snapshotDigest || !completion.primaryDigest || !completion.rewriteDigest || !completion.primaryCounts || !Number.isInteger(completion.primaryCounts.files) || !Number.isInteger(completion.primaryCounts.commits) || !Number.isInteger(completion.primaryCounts.recoveries) || !Number.isInteger(completion.primaryCounts.rewrites) || !Number.isInteger(completion.rewriteCount) || completion.rewriteCount < 0 || !Number.isInteger(completion.recoveriesPreserved) || completion.recoveriesPreserved < 0 || completion.primaryCounts.rewrites !== completion.rewriteCount || (!completion.force && completion.primaryCounts.recoveries !== completion.recoveriesPreserved) || (completion.force && completion.recoveriesPreserved !== 0) || !Array.isArray(completion.rewriteRecords) || completion.rewriteRecords.length !== completion.rewriteCount || !Array.isArray(completion.recoveryRecords) || completion.recoveryRecords.length !== completion.recoveriesPreserved || typeof completion.rewriteCompleted !== "boolean" || !["finalizing", "finalized"].includes(completion.phase)) return false;
...
async function deletePrimary(journal: RemovalJournal, scope: RepositoryOperationScope): Promise<void> {
  const db = await openPrimary(); await new Promise<void>((resolve, reject) => { const stores = ["repositories", "files", "commits", "logs", "repositoryDiagnostics", "maintenanceFences", ...(journal.force ? ["recoveries"] : [])]; const tx = db.transaction(stores, "readwrite"); const repositories = tx.objectStore("repositories"); const request = repositories.get(journal.repoId); let validationError: Error | null = null; request.onsuccess = () => { const current = request.result as LocalRepositoryMeta | undefined; try { assertRepositoryOperationScopeCurrent(scope); } catch (error) { validationError = error as Error; tx.abort(); return; } if (current && (current.localInstanceId !== journal.localInstanceId || (current.operationFence ?? 0) !== journal.removalFence)) { validationError = new RepositoryOwnershipChangedError("A replacement repository cannot be removed by this journal."); tx.abort(); return; } if (current) repositories.delete(journal.repoId); for (const name of ["files", "commits", "logs", ...(journal.force ? ["recoveries"] : [])]) { const store = tx.objectStore(name); const cursor = store.index("repoId").openKeyCursor(IDBKeyRange.only(journal.repoId)); cursor.onsuccess = () => { if (cursor.result) { store.delete(cursor.result.primaryKey); cursor.result.continue(); } }; cursor.onerror = () => tx.abort(); } const diagnostics = tx.objectStore("repositoryDiagnostics"); const diagnosticCursor = diagnostics.index("localInstanceId").openKeyCursor(IDBKeyRange.only(journal.localInstanceId)); diagnosticCursor.onsuccess = () => { const cursor = diagnosticCursor.result; if (cursor) { diagnostics.delete(cursor.primaryKey); cursor.continue(); } }; diagnosticCursor.onerror = () => tx.abort(); }; request.onerror = () => tx.abort(); tx.oncomplete = () => resolve(); tx.onerror = () => reject(validationError ?? tx.error); tx.onabort = () => reject(validationError ?? tx.error ?? new Error("Primary maintenance deletion was aborted.")); }); db.close();
}
