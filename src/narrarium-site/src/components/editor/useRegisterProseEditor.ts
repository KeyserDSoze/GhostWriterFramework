...
export function useRegisterProseEditor(
  ...
  handlers: { improve: (selection: string | null) => void; synonym: (selection: string) => void; enabled?: boolean },
) {
  const register = useProseEditorStore((s) => s.register);
  useEffect(() => {
    const el = ref.current;
    if (!el || handlers.enabled === false) return;
    return register({ el, improve: handlers.improve, synonym: handlers.synonym });
