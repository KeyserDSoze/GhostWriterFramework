...
describe("provider output limits", () => {
  ...
  it("sends the configured output limit for text completions", async () => {
    ...
    await completeText(integration, [{ role: "user", content: "write" }], "writing", { modelName: "model", maxOutputTokens: 321 });
  ...
  it("uses max_completion_tokens for modern reasoning models", async () => {
    mocks.create.mockResolvedValue({ choices: [{ message: { content: "done" }, finish_reason: "stop" }] });
    await completeText(integration, [{ role: "user", content: "reason" }], "writing", { modelName: "o3-mini", maxOutputTokens: 222 });
    ...
    await completeText(azure, [{ role: "user", content: "reason" }], "writing", { modelName: "private-review-deployment", underlyingModel: "o3-mini", maxOutputTokens: 111 });
    expect(mocks.create.mock.calls[1][0]).toMatchObject({ max_completion_tokens: 111 });

    await completeText(azure, [{ role: "user", content: "reason" }], "writing", { modelName: "o3-looking-deployment", maxOutputTokens: 99 });
  ...
  it("sends the configured output limit for forced-tool completions", async () => {
    ...
    await completeToolWith(integration, "model", undefined, [{ role: "user", content: "score" }], "review", { name: "score", description: "score", parameters: {} }, { maxOutputTokens: 123 });
