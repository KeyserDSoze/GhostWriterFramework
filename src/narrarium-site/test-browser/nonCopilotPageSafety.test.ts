...
describe("non-Copilot page safety contracts", () => {
  ...
  it("keeps expired offline sessions usable and protects destructive settings actions", () => {
    const auth = source("src/components/auth/AuthGuard.tsx");
    const settings = source("src/pages/SettingsPage.tsx");
    expect(auth).toContain('if (status === "offline")');
    expect(auth).toContain("return <>{children}</>");
    const settingsHook = source("src/drive/useSettings.ts");
    expect(settingsHook).toContain("loadCachedSettingsForHydration(expectedIdentity)");
    expect(settingsHook).toContain("reconcileCachedSettings(expectedIdentity");
    expect(settingsHook).toContain('reconciliation.kind === "conflict"');
    expect(source("src/components/layout/Topbar.tsx")).toContain("clearOfflineCache();");
    ...
    expect(settings).toContain("credentialsDisabled={offline}");
    expect(source("src/pages/AddBookPage.tsx")).toContain("disabled={offline}");
    expect(source("src/pages/BookSettingsPage.tsx")).toContain("disabled={offline}");
  });
});
