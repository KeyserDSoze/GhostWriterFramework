...
describe("LLM routed debug metadata", () => {
  ...
  it("records converted timeouts and fallback identity without retaining diagnostics", async () => {
    ...
    await expect(runWithCandidateTimeout((signal) => completeText(integration, [{ role: "user", content: "private manuscript" }], "writing", { signal, modelName: "gpt-4o", routeCandidateIndex: 1, usedFallback: true }), undefined, 5)).rejects.toMatchObject({ name: "CandidateTimeoutError" });
    const entry = useLlmDebugStore.getState().entries[0];
  ...
  it("classifies a provider AbortError as provider failure without an aborted operation signal", async () => {
    ...
    await expect(completeText(integration, [{ role: "user", content: "private" }], "writing", { modelName: "gpt-4o" })).rejects.toMatchObject({ name: "AbortError" });
