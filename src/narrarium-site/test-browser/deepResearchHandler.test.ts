...
describe("deep research Copilot adapter", () => {
  it("passes routed inputs, progress and cancellation to the persistence engine", async () => {
    ...
    const result = await executeDeepResearchFromCopilot({ prompt: "run deep research on Roman roads using wikipedia", settings: { ui: { language: "en" } } as any, structureLanguage: "it", book: { id: "book" } as any, branch: "draft", token: "token", accountScope: null, signal: controller.signal, onText: progress }, runner);
